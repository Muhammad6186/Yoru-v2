module.exports = function ({ api, models }) {
  ...
  logger(
    `${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${!global.config.BOTNAME ? "This bot was made by CatalizCS, SpermLord, Matthew And John Arida" : global.config.BOTNAME}`,
    "[ BOT INFO ]",
  );
  ...
  return (event) => {
    if (event.type == "change_thread_image")
      api.sendMessage(`» [ GROUP UPDATES ] ${event.snippet}`, event.threadID);
    let data = JSON.parse(
      fs.readFileSync(
        __dirname + "/../modules/commands/cache/approvedThreads.json",
      ),
    );
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        adminBot.forEach((e) => {
          api.sendMessage(
            `» ID: ${event.threadID}\n» 𝗥𝗲𝗾𝘂𝗲𝘀𝘁𝗲𝗱 𝗙𝗼𝗿 𝗔𝗽𝗽𝗿𝗼𝘃𝗮𝗹! `,
            e,
          );
        });
        return api.sendMessage(
          `𝗬𝗼𝘂𝗿 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝗛𝗮𝘀 𝗕𝗲𝗲𝗻 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗦𝗲𝗻𝘁 𝗧𝗼 𝗧𝗵𝗲 𝗔𝗱𝗺𝗶𝗻𝘀, 𝗣𝗹𝗲𝗮𝘀𝗲 𝗪𝗮𝗶𝘁 𝗙𝗼𝗿 𝗧𝗵𝗲𝗺 𝗧𝗼 𝗔𝗽𝗽𝗿𝗼𝘃𝗲 𝗜𝘁 𝗙𝗶𝗿𝘀𝘁, 𝗕𝗲𝗳𝗼𝗿𝗲 𝗨𝘀𝗶𝗻𝗴 𝗠𝗲, 𝗧𝗵𝗮𝗻𝗸 𝗬𝗼𝘂!`,
          event.threadID,
        );
      }
      if (event.body && event.body.startsWith(prefix))
        return api.sendMessage(
          `𝗬𝗼𝘂𝗿 𝗕𝗼𝘅 𝗛𝗮𝘀 𝗡𝗼𝘁 𝗕𝗲𝗲𝗻 𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱 𝗬𝗲𝘁. 𝗧𝗼 𝗦𝘂𝗯𝗺𝗶𝘁 𝗔 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝗙𝗼𝗿 𝗔𝗽𝗽𝗿𝗼𝘃𝗮𝗹, 𝗣𝗹𝗲𝗮𝘀𝗲 𝗧𝘆𝗽𝗲 𝗢𝗻 𝗬𝗼𝘂𝗿 𝗧𝗵𝗿𝗲𝗮𝗱: ${prefix}request\n\n-‘๑’-𝗔𝗱𝗺𝗶𝗻 𝗙𝗕  𝗟𝗶𝗻𝗸:\n https://www.facebook.com/100027867581039`,
          event.threadID,
        );
    }
    switch (event.type) {
